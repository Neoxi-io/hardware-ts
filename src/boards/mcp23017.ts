...
export class MCP23017 extends IOExpander<IOExpander.PinNumber16> {
   ...
   */
  constructor(i2cBus: I2CBus, address: number, debug: boolean = false) {
    super(i2cBus, address)

    this.debug = debugFactory('MCP23017')

    if (debug) {
      debugFactory.enable('MCP23017')
    }
  }
  ...
  protected async _initializeChip(initialHardwareState: number): Promise<void> {
    ...
    const ioconFlags = MCP23017_IOCON_FLAGS.DEFAULT | MCP23017_IOCON_FLAGS.INT_MIRROR_ON | MCP23017_IOCON_FLAGS.INT_OPEN_DRAIN_ENABLED
...
export class MCP23017A extends IOExpander<IOExpander.PinNumber8> {
  ...
  protected async _initializeChip(initialHardwareState: number): Promise<void> {
    ...
    const ioconFlags = MCP23017_IOCON_FLAGS.DEFAULT | MCP23017_IOCON_FLAGS.INT_OPEN_DRAIN_ENABLED
